...
| **2** | [**Raspberry Pi File Server (Samba)**](./labs/lab2-file-server-samba/README.md) | Build a local network file server using Samba, create shared directories, manage UNIX/Samba users, configure access controls, and test client access from macOS and Windows. Includes directory permissions, service persistence, and troubleshooting connectivity issues. |
